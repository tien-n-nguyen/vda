...
given at all. 
Also, the VA tool fails to learn from assessment patterns within commits that are vulnerable, but missed by the VD tool.
As a result, the VD $\rightarrow$ VA strategy would not
...
assessment at the same time, leading to performance enhancement in
both tasks.

Second, the state-of-the-art VA approaches~\cite{deepCVA-ase21} are
still limited in their {\em representation of code changes}, 
...
representation for code changes that only captures limited local context,
but is incapable of capturing VD/VA-relevant program statements that
...
This facet is ignored in an $n$-gram representation learning approach.
...
%impact grades. 
Finally, in the state-of-the-art VA tool, the context of
...
%{\tool} provides assessment scores when a vulnerability is detected. 
