...
in assessment. The corresponding decrease values for the model without
Context are 11.6\% F-score in detection and 10.9\% macro F-score in
...
performance significantly. This result confirms our hypotheses. (1)
...
also improves over the baselines (Tables~\ref{tab:rq1} and
~\ref{rq1_results}) in both VD and VA due to its better code change
embeddings. (2) multi-task learning performs much better than
detection-then-assessment scheme (VD $\rightarrow$ VA). (3) both
multi-task learning and context have positive contributions to
{\tool}.
