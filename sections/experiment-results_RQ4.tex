...
performance decreases 16\% F-score in detection and 18.8\% macro
F-score in assessment. The corresponding decrease values for the model
without Context are 11.6\% F-score in detection and 10.9\% macro
F-score in assessment. Thus, multi-task learning contributes more than
...
classifiers for assessment will be used (i.e., VD $\rightarrow$
VA). As seen, the cascading error due to false positives reduce the
performance significantly. This result confirms our hypotheses that 1)
multi-task learning helps improve both vulnerability detection and
assessment, 2) multi-task learning performs much better
than detection-then-assessment scheme, and 3) both multi-task learning
and context have positive contributions to {\tool}.
