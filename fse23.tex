...
\title[{\tool}: Commit-level Vulnerability Detection and Assessment]{{\tool}: Commit-level Vulnerability Detection and Assessment}
...
detects any vulnerability and provides the CVSS assessment grades for
it. To build {\tool}, we have key novel components. First, we design a
novel context-aware, graph-based, representation learning model to
learn the {\em contextualized embeddings for the code changes} that
...
assessment. Second, {\tool} considers the mutual impact of the
learning to detect vulnerability and the learning to assess each of
...
assessment tasks.
%during the assessment of one security aspect, also considers the
%impacts of the other aspects by leveraging multi-task learning
%(each~task learning to assess one aspect) to propagate the learning
%from one task to another.
Our empirical evaluation shows that on a vulnerability dataset in C,
{\tool} achieves F-score of 25.5\% and MCC of 26.9\% relatively higher
than the state-of-the-art baseline models in vulnerability
assessment. In a Java dataset, {\tool} achieves F-score of 20.4\% and
MCC of 28.0\% relatively higher than the baselines as well. {\tool} also
improves the vulnerability detection over the existing approaches from
